...
test <- test %>% 
   dplyr::select(-`Preventive_Action_Taken_Scale_PPEuse_Total`)
```
...
```{r}
# get predictions corresponding to model fit to training data using optimal lambda
frac <- cvlas$fraction[which.min(cvlas$cv.error)]
frac  # this is returning a null result for some reason
